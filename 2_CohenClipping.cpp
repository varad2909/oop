...
#include <dos.h>
 ...
 using namespace std; //remove nameing conflict
 
 static int LEFT=1,RIGHT=2,BOTTOM=4,TOP=8,xl,yl,xh,yh;

int getcode(int x,int y) // a---------b
{
	int code = 0;
...
//	int x1,y1,x2,y2;
	...
	while(1){
		float m =(float)(y2-y1)/(x2-x1); //slop calculation
		//Both points inside. Accept line
		if(outcode1==0 && outcode2==0){ 
			accept = 1; //inside the window
			break;
		}
...
//Decide if point1 is inside, if not, calculate intersection
			...
			//Line clips top edge
			if(temp & TOP){
				x = x1+ (yh-y1)/m;   //intersection Formula
				y = yh;
			}
